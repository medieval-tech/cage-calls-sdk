...
export const cageCallsQueryKeys = Object.freeze({
  ...
  fightEvent: (eventName: string, input?: { seasonId?: bigint; fightIds?: readonly bigint[]; viewer?: Address; expectedFightCount?: number; cursor?: bigint; limit?: number; now?: bigint }) => key("fight-event", eventName, input?.seasonId?.toString() ?? "all", ...(input?.fightIds?.map(String) ?? ["discover"]), input?.viewer ? normalizeAddress(input.viewer) : "none", input?.expectedFightCount ?? "unknown", input?.cursor?.toString() ?? "start", input?.limit ?? "default", input?.now?.toString() ?? "clock"),
  ...
  accountFightStates: (account: Address, input?: { limit?: number; cursor?: bigint }) => key("account-fight-states", normalizeAddress(account), input?.limit ?? "default", input?.cursor?.toString() ?? "start"),
  ...
  relicInventory: (input?: RelicCollectionInput) => key("relics", "inventory", input?.pageSize ?? "default", input?.enrichFighters ?? true),
  ...
  ownedRelicInventory: (account: Address) => key("owned-relic-inventory", normalizeAddress(account)),
  gacha: (fightId: bigint) => key("gacha", fightId.toString()),
  gachaPools: (fightIds: readonly bigint[]) => key("gacha", "pools", ...fightIds.map(String)),
  gachaUser: (fightId: bigint, account: Address) => key("gacha-user", fightId.toString(), normalizeAddress(account)),
  gachaUsers: (fightIds: readonly bigint[], account: Address) => key("gacha-users", normalizeAddress(account), ...fightIds.map(String)),
