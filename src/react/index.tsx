...
const scopedKeysCache = new WeakMap<object, typeof baseKeys>();
...
export function useAdminCapabilities(account: Address, options?: Options<DataResult<AdminCapabilities>>) {
  const client = useCageCallsClient();
  const keys = queryKeysFor(client);
  return useQuery({
    queryKey: keys.admin(`capabilities:${normalizeAddress(account)}`),
    queryFn: ({ signal }) => client.admin.capabilities(account, { signal }),
    staleTime: 30_000,
    refetchOnWindowFocus: false,
    ...options,
  });
...
function liveInvalidationKeys(keys: typeof baseKeys, update: CageCallsLiveUpdate): readonly (readonly unknown[])[] {
  switch (update.kind) {
    ...
    case "reconcile": return [keys.all()];
  }
}
  ...
  useEffect(() => {
    ...
    void client.live.subscribe(filter, {
      update(update) {
        ...
        for (const queryKey of liveInvalidationKeys(keys, update)) void queryClient.invalidateQueries({ queryKey });
      },
